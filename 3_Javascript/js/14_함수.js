...
// 2개의 값을 전달받아 출력하는 함수
function print1(num, str) { // 함수의 선언
  // 함수 정의
...
btn1.addEventListener("click", function() {
  ...
  // 함수 호출(값 전달)
  // 중요!!!!! 전달되는 값의 순서가 중요
  // 함수명을 Crtl + 좌클릭시 해담 함수 줄로 이동함
  print1(Number(value), result);
  // print1(result, Number(value));
...
document.querySelector("#btn2a").addEventListener("click", function(){
  ...
  // arrayTest() 함수에서 arr2와 같은 배열을 참조햇 수전
  // -> 함수 수행 후 돌아와서도
  // 참조하고 있던 배열의 2번 인덱스 값이 변환
...
document.querySelector("#btn3a").addEventListener("click", function(){
  ...
  numbers.push(50);
  numbers.push(4);
  numbers.push(returnTest(2, 5)); // 함수를 호출해서 되돌려 받은 값을 push 하기
...
function twoNumberPlus( otherFn ) {
  ...
  alert(`합계 : ${otherFn(input1, input2)}`);
...
arrowList[2].addEventListener("click", () => {
  ...
  // (a, b) => a + b 정의된 상태이고 전달인자로 넘겨지는 것 호출이 아님
  // return 한 줄만 있는경우
  // {}, return 생략 가능
