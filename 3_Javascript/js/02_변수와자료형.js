...
//console.log(문자열); 홑따옴표 쌍따옴표없이 쓰는 문자는 변수로 취급
...
console.log(number1);// 값을 초기화전에는 정의되지 않아서 출력이 안 됨. undefined
...
console.log("number1 :", number1, "/ number2 :", number2); // "number1 : " 300
...
// 같은 이름으로(변수명) 선언을 또다시 하려고 해서 에러남
...
// -> 둘다 블록 레벨 스코프이기 때문에 블록 밖에서 접근할 수 없음
...
console.log(test1);
console.log(test2); //블록 레벨을 무시하고 전역변수로 인식하여 블록 밖에서도 가능
// - > 가능 .. 왜? 함수레벨 스코프인 var는 블록을 무시하기 때문에

// 그럼 함수 안에서는 어떻게 될까?

// 함수의 선언
function example() { // 이렇게 함수 형태로 만들어진 코드 블록은 함수레벨 scope
...
// var와 달리 let/const로 선언된 변수는 초기화가 이루어지기 전에는 사용 될 수 없음.

/*
* TDZ(Temporal Dead Zone)
* : 시간적으로 죽은 구역..(코드의 사각지대)
* - 코드실행과정에서 특정 시점까지 변수가 접근 불가능한 상태를 의미함
...
// undefined: 정의되지 않은 변수나 값 -> 값이 아직 대입되지 않았다
let undef; // 변수의 선언
...
// number(정수, 실수, 양수, 음수, 0 등 모든숫자)
const age = 20;
const height = 183.2;

console.log("age : ", age, typeof age, ", height : ", height, typeof height);
...
// 1) 배열(Array) : 여러 값이 나열되어 있는 것으의 묶음
const numbers = [10, 20, 30];//JS의 배역은 [] 작성
console.log("numbers : " , numbers, typeof numbers);

console.log("numbers 배역 값 중 0번째" , numbers[0], typeof numbers[0]);//type이 number인 이유는 배열 속 값의 타입이 number이기 때문
console.log("numbers 배역 값 중 1번째" , numbers[1], typeof numbers[1]);
console.log("numbers 배역 값 중 2번째" , numbers[2], typeof numbers[2]);
// console.log("numbers 배역 값 중 3번째" , numbers[3], typeof numbers[3]);
// undefined - numbers[3] 에는 아무값도 할당되지 않아 nudifined가 된다. 선언은 되어있고 초기화 되어있지 않은 상태

// 2) JS 객채
// 값을 K:V (Map) 형식으로 여러 갭 저장하는 형태
// -> { K:V, K:V, K:V ... }

// K(key) : 값을 구분하는 이름(변수명 비슷)
// V(vlaue) : K에 대응되는 값 (변수에 대입되는 값 비슷)
...
console.log("user['name'] : ", user['name'], typeof user['name']);
